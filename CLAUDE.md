...
- **Pendência conhecida, fora do escopo desta tarefa**: `produtosRepository.getLucroPorProduto` calcula `faturamento` com `iv.quantidade * p.preco_venda` (preço **atual**), não `iv.preco_unitario` (preço **travado** na venda) — mesma mutabilidade histórica que esta migration corrigiu do lado do custo, só que do lado do preço. Só custo foi pedido nesta tarefa; preço fica para uma correção separada.
